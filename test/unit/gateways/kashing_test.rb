...
class KashingTest < Test::Unit::TestCase
  include CommStub
  ...
  def setup
    @gateway = KashingGateway.new(
      merchant_id: '165',
      merchant_key: '6470-1c65-f70d-664c-2df1-ae83'
    )
  ...
  def test_successful_purchase
    ...
    result = response.params['results'][0]
    assert_equal 4, result['responsecode'].to_i
    assert_equal 1, result['reasoncode'].to_i
    assert_equal 'GBP', result['currency']
    # assert_equal @amount, result['amount'].to_i
    assert response.test?
  end
